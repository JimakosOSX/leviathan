...
#### from here onwards, custom functions are added
get_IPs() {

	IPs=$(ip -f inet a | awk '/inet / { print $2 }' | cut -d '/' -f1)
	log IPs "${IPs}" >&6
}

get_update() {

	update=$(TZ="Europe/Athens" date "+%d/%m/%Y %R")
	log update "${update}" >&6
}

get_root_percent() {

	root_percent=$(df -h / --output=pcent | tail -1)
	log root_percent "${root_percent}" >&6
}

get_load_avg() {

	load_avg=$(uptime | cut -d ',' -f4,5,6 | sed -e 's/^[ \t]*//')
	log load_avg "${load_vg}" >&6
}
...
EOF
    ...
    # shellcheck disable=2086
    {
        ...
        # contents of 'PF_INFO'.
        set -f
        set +f -- ${PF_INFO-ascii title os host kernel uptime pkgs memory update load_avg root_percent IPs}
